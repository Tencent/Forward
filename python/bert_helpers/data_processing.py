...
def convert_example_to_features(doc_tokens, question_text, tokenizer,
    ...
    _Feature = collections.namedtuple(  # pylint: disable=invalid-name
        "Feature", [
            "input_ids", "input_mask", "segment_ids", "tokens",
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
        ...
        features.append(
            _Feature(input_ids=create_int_feature(input_ids),
...
def read_squad_json(input_file):
    ...
    _Example = collections.namedtuple(  # pylint: disable=invalid-name
        "Example", ["id", "question_text", "doc_tokens"])
...
def get_predictions(doc_tokens, features, results, n_best_size,
                    max_answer_length):
    ...
    if version_2_with_negative:
        prelim_predictions.append(
            _PrelimPrediction(feature_index=result.feature_index,
                              start_index=0,
                              end_index=0,
