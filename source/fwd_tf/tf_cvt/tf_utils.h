...
class Utils {
 public:
   ...
   */
  template <typename T>
  static std::shared_ptr<TF_Tensor> CreateRandomTensor(TF_DataType data_type,
    ...
    return CreateTensor(data_type, dims, data.data());
  }

  template <typename T>
  static std::shared_ptr<TF_Tensor> CreateRandomIntTensor(TF_DataType data_type,
                                                          const std::vector<int64_t>& dims,
                                                          int max = 10000) {
    auto size = std::accumulate(dims.begin(), dims.end(), 1, std::multiplies<std::int64_t>());
   ...
   */
  static std::shared_ptr<TF_Session> CreateSession(
  ...
  static std::vector<int64_t> GetTensorShape(const TF_Tensor* tensor) {
  ...
  static size_t GetElementSize(TF_DataType type) {
    switch (type) {
      case TF_INT64:
        return 8;
      case TF_INT32:
  ...
  static std::shared_ptr<TF_Tensor> CreateEmptyTensor(TF_DataType data_type,
                                                      const std::vector<int64_t>& dims) {
   ...
   */
  template <typename T>
  static std::shared_ptr<TF_Tensor> CreateTensor(TF_DataType data_type,
   ...
   * \return
   */
  static TF_Code DeleteSession(TF_Session* session) {
    if (session == nullptr) {
   ...
   */
  static nvinfer1::Dims DimsOf(const TF_Tensor* tensor) {
  ...
  static nvinfer1::Dims DimsOf(const Output& output) {
    Status status;
  ...
  static nvinfer1::Dims DimsOf(const Operation& op) {
    return DimsOf(Output(op.Graph(), op.Op(), 0));
  }
  ...
  static std::vector<std::vector<T>> GetTensorsData(
      const std::vector<std::shared_ptr<TF_Tensor>>& tensors) {
    std::vector<std::vector<T>> data;
    data.reserve(tensors.size());
    for (auto t : tensors) {
   ...
   * \return
   */
  static TF_Buffer* ReadBufferFromFile(const std::string& filename) {
   ...
   */
  static bool RunSession(TF_Session* session, const TF_Output* inputs,
   ...
   */
  static FwdWeights ToFwdWeights(TF_Tensor* tf_tensor) {
    FwdWeights weights;
  ...
  static FwdWeights ToFwdWeights(const Tensor& tensor) { return ToFwdWeights(tensor.get()); }

  static TF_DataType TfDataType(DataType type) {
    switch (type) {
      case DataType::HALF:
        return TF_HALF;
      case DataType::INT32:
  ...
  static nvinfer1::DataType NvDataType(const TF_DataType& type) {
    switch (type) {
      case TF_HALF:
...
template <>
inline std::shared_ptr<TF_Tensor> Utils::CastTensor<uint16_t>(const TF_Tensor* tensor,
