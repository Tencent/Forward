...
/**
 * \brief Softmax 层描述创建器
 */
template <>
class TLayerDescCreator<TrtSoftmaxDesc> : public ILayerDescCreator {
 public:
  ...
  std::shared_ptr<TrtLayerDesc> Create(const Layer& layer, const H5ModelReader& reader,
    ...
    LOG(INFO) << "TrtSoftMax::Create";

    input_names = layer.Inputs();

    auto layer_desc = std::make_shared<TrtSoftmaxDesc>();
    const auto axes = layer.GetAttr<int>("axis");

    // TODO(Ao Li): 目前只支持 NCHW softmax
    layer_desc->axes = 1 << TrtUtils::NHWC2NCHWDim(axes >= 0 ? axes : axes + 4);
    return layer_desc;
  }
};
