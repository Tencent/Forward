...
class TLayerDescCreator<TrtClampDesc> : public ILayerDescCreator {
 public:
  bool Check(const Layer& layer) override { return layer.Type() == "ReLU"; }
  ...
  std::shared_ptr<TrtLayerDesc> Create(const Layer& layer, const H5ModelReader& reader,
    ...
    const float max_value = layer.GetAttr<float>("max_value");
    const float negative_slop = layer.GetAttr<float>("negative_slope");
    const float threshold = layer.GetAttr<float>("threshold");

    // TODO(Ao Li): 暂时不支持 negative slop 不为 0 的情况
    T_CHECK_EQ(negative_slop, 0.0);

    auto layer_desc = std::make_shared<TrtClampDesc>();

    layer_desc->max = max_value;
    layer_desc->min = threshold;
