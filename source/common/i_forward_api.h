...
class IForwardBuilder {
 public:
  ...
  // InferMode should be set before Build. The default value is InferMode::FLOAT.
  virtual void SetInferMode(InferMode mode) = 0;

  // opt_batch_size should be set before Build. If not set, the opt_batch_size is set as
  // the max_batch_size given by dummy inputs. The engine will try to optimized itself for the
  // input with the opt_batch_size.
