 ...
 */
template <>
class TLayerCreator<TrtGeluDesc> : public ILayerCreator {
 public:
  ITensorVector CreateLayer(nvinfer1::INetworkDefinition* network, const TrtLayerDesc* layer_desc,
    ...
    LOG(INFO) << "TrtGeluDesc::CreateLayer";
    const auto gelu_desc = dynamic_cast<const TrtGeluDesc*>(layer_desc);
    T_CHECK(gelu_desc);

    const auto input = input_tensors[0];
    return {bert::CreateGeluLayer(network, input, gelu_desc->use_fp16, gelu_desc->use_int8)};
  }
};
