...
EmbLayerNormPluginDynamic::EmbLayerNormPluginDynamic(const std::string& name,
                                                     ...
                                                     const nvinfer1::DataType mhaType,
                                                     const Weights& beta, const Weights& gamma,
                                                     const Weights& wordEmb, const Weights& posEmb,
                                                     const Weights& tokEmb, const bool useFullMask)
    : mLayerName(name), mLd(beta.count), mType(type), mUseFullMask(useFullMask), mMhaType(mhaType) {
...
EmbLayerNormPluginDynamic::EmbLayerNormPluginDynamic(const std::string& name, const void* data,
                                                     size_t length)
    : mLayerName(name),
      mGammaDev(nullptr),
      mBetaDev(nullptr),
      ...
      mTokEmbDev(nullptr),
      mPosEmbDev(nullptr) {
  LOG(INFO) << "EmbLayerNormPluginDynamic deserialize\n";
...
IPluginV2DynamicExt* EmbLayerNormPluginDynamic::clone() const {
  LOG(INFO) << "EmbLayerNormPluginDynamic clone\n";

  auto p = new EmbLayerNormPluginDynamic(mLayerName, mType, mMhaType, mBeta, mGamma, mWordEmb,
                                         mPosEmb, mTokEmb, mUseFullMask);
  p->mS = mS;
...
DimsExprs EmbLayerNormPluginDynamic::getOutputDimensions(int outputIndex, const DimsExprs* inputs,
  ...
  // this code must match getMHAMaskPackedSize in bert_common.h
  if (mUseFullMask ||
    ...
    auto is0 =
        exprBuilder.operation(DimensionOperation::kEQUAL, *maskSize, *exprBuilder.constant(0));
...
bool EmbLayerNormPluginDynamic::supportsFormatCombination(int pos, const PluginTensorDesc* inOut,
  ...
  if (pos == 0) {
    return desc.type == nvinfer1::DataType::kINT32 && desc.dims.nbDims == 2;
  }
...
void EmbLayerNormPluginDynamic::configurePlugin(const DynamicPluginTensorDesc* inputs, int nbInputs,
                                                ...
                                                int nbOutputs) {
  LOG(INFO) << "EmbLayerNormPluginDynamic configurePlugin\n";
  ...
  assert(inputs[1].desc.type == nvinfer1::DataType::kINT32);
  assert(inputs[2].desc.type == nvinfer1::DataType::kINT32);
  assert(outputs[0].desc.type == mType);
  assert(outputs[1].desc.type == nvinfer1::DataType::kFLOAT);
}
...
int EmbLayerNormPluginDynamic::enqueue(const PluginTensorDesc* inputDesc,
  ...
  if (mType == nvinfer1::DataType::kFLOAT) {
    ...
    embSkipLayerNorm<float>(stream, static_cast<int>(mLd), batchSize, S, inputIds, segmentIds, beta,
    ...
    embSkipLayerNorm<half>(stream, static_cast<int>(mLd), batchSize, S, inputIds, segmentIds, beta,
                           gamma, wordEmb, posEmb, tokEmb, output);
  } else {
    LOG(ERROR) << "Unsupported type error, expected [kHALF,kFLOAT], but received "
...
nvinfer1::DataType EmbLayerNormPluginDynamic::getOutputDataType(
    int index, const nvinfer1::DataType* inputTypes, int nbInputs) const {
  assert(index == 0 || index == 1);
  if (index == 0) {
    assert(mType == nvinfer1::DataType::kHALF || mType == nvinfer1::DataType::kFLOAT);
    return mType;
  }
...
int EmbLayerNormPluginDynamic::initialize() { return 0; }

void EmbLayerNormPluginDynamic::terminate() {
  LOG(INFO) << "EmbLayerNormPluginDynamic terminate\n";
}
...
void EmbLayerNormPluginDynamic::destroy() {
  LOG(INFO) << "EmbLayerNormPluginDynamic destroy\n";
...
const char* EmbLayerNormPluginDynamic::getPluginNamespace() const { return mNamespace.c_str(); }
...
const char* EmbLayerNormPluginDynamicCreator::getPluginName() const {
  return FWD_EMB_LAYER_NORM_NAME;
}
...
IPluginV2* EmbLayerNormPluginDynamicCreator::createPlugin(const char* name,
                                                          const PluginFieldCollection* fc) {
  LOG(INFO) << "EmbLayerNormPluginDynamic createPlugin\n";
  ...
  for (int i = 0; i < fc->nbFields; i++) {
    ...
    if (field_name.compare("embedding_layernorm_beta") == 0) {
      LOG(INFO) << "Building bert_embeddings_layernorm_beta...\n";
      beta.values = fc->fields[i].data;
      beta.count = fc->fields[i].length;
      beta.type = fieldTypeToDataType(fc->fields[i].type);
    }

    if (field_name.compare("embedding_layernorm_gamma") == 0) {
      LOG(INFO) << "Building bert_embeddings_layernorm_gamma...\n";
      gamma.values = fc->fields[i].data;
      gamma.count = fc->fields[i].length;
      gamma.type = fieldTypeToDataType(fc->fields[i].type);
    }

    if (field_name.compare("embedding_word_embedding") == 0) {
      LOG(INFO) << "Building bert_embeddings_word_embeddings...\n";
      word_emb.values = fc->fields[i].data;
      word_emb.count = fc->fields[i].length;
      word_emb.type = fieldTypeToDataType(fc->fields[i].type);
    }

    if (field_name.compare("embedding_token_type_embedding") == 0) {
      LOG(INFO) << "Building bert_embeddings_token_type_embeddings...\n";
      tok_emb.values = fc->fields[i].data;
      tok_emb.count = fc->fields[i].length;
      tok_emb.type = fieldTypeToDataType(fc->fields[i].type);
    }

    if (field_name.compare("embedding_position_embedding") == 0) {
      LOG(INFO) << "Building bert_embeddings_position_embeddings...\n";
      pos_emb.values = fc->fields[i].data;
      pos_emb.count = fc->fields[i].length;
      pos_emb.type = fieldTypeToDataType(fc->fields[i].type);
    }
    if (field_name.compare("use_fp16") == 0) {
      LOG(INFO) << "Building output_fp16...\n";
      assert(fc->fields[i].type == PluginFieldType::kINT32);
      output_fp16 = static_cast<const int*>(fc->fields[i].data)[0] != 0;
    }
    if (field_name.compare("use_full_mask") == 0) {
      LOG(INFO) << "Building full_mask...\n";
      assert(fc->fields[i].type == PluginFieldType::kINT32);
    ...
    if (field_name.compare("mha_type_id") == 0) {
      ...
      LOG(INFO) << "Building mha typeId: " << mhaTypeId << std::endl;
    }
  }

  LOG(INFO) << "Building the Plugin...\n";
  nvinfer1::DataType mhaType = static_cast<nvinfer1::DataType>(mhaTypeId);
  EmbLayerNormPluginDynamic* p = new EmbLayerNormPluginDynamic(
      name, output_fp16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT, mhaType, beta,
      gamma, word_emb, pos_emb, tok_emb, useFullMask);
  return p;
}

IPluginV2* EmbLayerNormPluginDynamicCreator::deserializePlugin(const char* name,
