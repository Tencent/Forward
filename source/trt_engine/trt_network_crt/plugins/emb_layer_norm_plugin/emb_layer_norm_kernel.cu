...
#include "NvInfer.h"
#include "emb_layer_norm_plugin.h"
...
void convertMask(const uint32_t S, const uint32_t B, const uint32_t warps_m, const uint32_t warps_n,
  ...
  dim3 grid(xmmas_m, B);
  fillSBSMaskKernel<<<grid, threads_per_cta, S * sizeof(int), stream>>>(warps_m, warps_n, S,
                                                                        inputMaskSB, inputMaskX);
...
__global__ void embLayerNormKernel(int ld, const int* inputIds, const int* tokenIds,
                                   const float* beta, const float* gamma, const T* wordEmb,
...
int embSkipLayerNorm(cudaStream_t stream, int ld, int B, int S, const int* inputIds,
  ...
  embLayerNormKernel<T, tpb><<<grid, block, 0, stream>>>(ld, inputIds, token_ids, beta, gamma,
...
template int embSkipLayerNorm<float>(cudaStream_t, int, int, int, const int*, const int*,
                                     const float*, const float*, const float*, const float*,
                                     const float*, float*);

template int embSkipLayerNorm<half>(cudaStream_t, int, int, int, const int*, const int*,
                                    const float*, const float*, const half*, const half*,
                                    const half*, half*);
