...
//          Paul LU (lujq96@gmail.com)
 ...
 __inline__ __device__ float pow(float X, float Y) {
   return (Y == 1.0f) ? X : powf(X, Y);
 }
 ...
 __inline__ __device__ T BlockReduceSum(T val, T* shared) {
   ...
   __syncthreads();
   val = (threadIdx.x < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared[lid]
                                                                  : 0;
   if (wid == 0) {
     val = WarpReduceSum(val);
   }
 ...
 template <typename T>
 __global__ void TailReduceSum(int64_t PreDims, int64_t ReduceDims,
                               int64_t PostDims, const T* X, float power,
                               T* result) {
   using T_ACC = acc_type<T>;
   __shared__ T_ACC m_shared[WARP_SIZE];
 ...
 template <typename T>
 __global__ void NonTailReduceSum(int64_t PreDims, int64_t ReduceDims,
                                  int64_t PostDims, const T* X, float power,
                                  T* result) {
   using T_ACC = acc_type<T>;
   constexpr int vt = 4;
   ...
   for (int thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
        thread_idx < PreDims * PostDims; thread_idx += blockDim.x * gridDim.x) {
