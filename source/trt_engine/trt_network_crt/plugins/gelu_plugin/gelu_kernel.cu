...
int computeGelu(cudaStream_t stream, int n, const half* input, half* output) {
  ...
  if (0 == (n & 1)) {
    ...
    geluKernel<half2, blockSize>
        <<<gridSize, blockSize, 0, stream>>>(A2, B2, C2, n2, input2, output2);
  } else {
    const int gridSize = (n + blockSize - 1) / blockSize;
...
__global__ void geluBiasKernel(const T a, const T b, const T c, T* output, const T* input,
                               const T* bias, const int ld) {
