...
class MHARunner {
 public:
  ...
  virtual void run(const nvinfer1::PluginTensorDesc& inputDesc,
                   const nvinfer1::PluginTensorDesc& outputDesc, const void* qkvPtr,
                   const void* maskPtr, void* output, void* workspace, cudaStream_t stream) = 0;
...
class QKVToContextPluginDynamic : public nvinfer1::IPluginV2DynamicExt {
 public:
  QKVToContextPluginDynamic(const std::string name, const nvinfer1::DataType type,
                            const int hiddenSize, const int numHeads, const float dqProbs,
                            bool hasImask = false);

  QKVToContextPluginDynamic(const std::string name, const void* data, size_t length);

  // It doesn't make sense to make QKVToContextPluginDynamic without arguments,
  ...
  bool SupportINT8(const nvinfer1::PluginTensorDesc* in, int packedSize) const;

  bool CheckInputMask(int pos, const nvinfer1::PluginTensorDesc* inOut,
                      const nvinfer1::PluginTensorDesc* in, int packedSize);

  bool CheckOutput(const nvinfer1::PluginTensorDesc* in, const nvinfer1::PluginTensorDesc* out);
  bool CheckInput(const nvinfer1::PluginTensorDesc* in);
...
class QKVToContextVarSeqlenPlugin : public nvinfer1::IPluginV2DynamicExt {
 public:
  QKVToContextVarSeqlenPlugin(const std::string name, const nvinfer1::DataType type,
                              const int hiddenSize, const int numHeads, const float dqProbs,
                              bool hasImask = false, bool varSeqlen = false);

  QKVToContextVarSeqlenPlugin(const std::string name, const void* data, size_t length);

  // It doesn't make sense to make QKVToContextVarSeqlenPlugin without
  ...
  nvinfer1::DimsExprs getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs,
                                          int nbInputs,
                                          nvinfer1::IExprBuilder& exprBuilder) override;
  bool CheckFormats(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs,
                    const nvinfer1::PluginTensorDesc* in,
                    const nvinfer1::PluginTensorDesc* out) const;
  bool supportsFormatCombination(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs,
                                 int nbOutputs) override;
  void configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in, int nbInputs,
                       const nvinfer1::DynamicPluginTensorDesc* out, int nbOutputs) override;
...
class UnfusedMHARunner : public MHARunner {
 public:
  ...
  void run(const nvinfer1::PluginTensorDesc& inputDesc,
           const nvinfer1::PluginTensorDesc& outputDesc, const void* qkvPtr, const void* maskPtr,
