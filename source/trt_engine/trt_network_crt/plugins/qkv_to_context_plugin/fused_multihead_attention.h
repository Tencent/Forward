...
static const struct FusedMultiHeadAttentionKernelMetaInfoV1 {
  ...
  unsigned int mThreadsPerCTA;
    ...
    {DATA_TYPE_FP16, 128, 64, kSM_75, fused_multihead_attention_fp16_128_64_kernel_sm75_cu_o,
     ...
     "fused_multihead_attention_fp16_128_64_kernel_sm75", 32768, 128},
    {DATA_TYPE_FP16, 384, 64, kSM_75, fused_multihead_attention_fp16_384_64_kernel_sm75_cu_o,
     ...
     "fused_multihead_attention_fp16_384_64_kernel_sm75", 57344, 256},
    {DATA_TYPE_INT8, 128, 64, kSM_75, fused_multihead_attention_int8_128_64_kernel_sm75_cu_o,
     ...
     "fused_multihead_attention_int8_128_64_kernel_sm75", 16384, 128},
    {DATA_TYPE_INT8, 384, 64, kSM_75, fused_multihead_attention_int8_384_64_kernel_sm75_cu_o,
     ...
     "fused_multihead_attention_int8_384_64_kernel_sm75", 53284, 256},
    ...
    {DATA_TYPE_FP16, 128, 64, kSM_80, fused_multihead_attention_fp16_128_64_kernel_sm80_cu_o,
     ...
     "fused_multihead_attention_fp16_128_64_kernel_sm80", 49152, 128},
    {DATA_TYPE_FP16, 384, 64, kSM_80, fused_multihead_attention_fp16_384_64_kernel_sm80_cu_o,
     ...
     "fused_multihead_attention_fp16_384_64_kernel_sm80", 114688, 256},
    {DATA_TYPE_INT8, 128, 64, kSM_80, fused_multihead_attention_int8_128_64_kernel_sm80_cu_o,
     ...
     "fused_multihead_attention_int8_128_64_kernel_sm80", 24576, 128},
    {DATA_TYPE_INT8, 384, 64, kSM_80, fused_multihead_attention_int8_384_64_kernel_sm80_cu_o,
     ...
     "fused_multihead_attention_int8_384_64_kernel_sm80", 57344, 256},
...
class TFusedMultiHeadAttentionXMMAKernel {
 public:
  ...
  inline uint64_t hashID(unsigned int s, unsigned int d) const { return (uint64_t)s << 32 | d; }
  virtual uint64_t hashID(const KernelMeta& kernelMeta) const {
    return hashID(kernelMeta.mS, kernelMeta.mD);
  }

  TFusedMultiHeadAttentionXMMAKernel(const TKernelMeta* pMetaStart, unsigned int nMetaCount,
                                     Data_type type, unsigned int sm)
  ...
  void loadXMMAKernels() {
    ...
    for (unsigned int i = 0; i < mKernelMetaCount; ++i) {
      ...
      if (kernelMeta.mSM == mSM && kernelMeta.mDataType == mDataType) {
        ...
        cuErrCheck(
            mDriver.cuModuleGetFunction(&funcInfo.mDeviceFunction, hmod, kernelMeta.mFuncName),
            mDriver);
        ...
        if (mValidSequences.find(s) == mValidSequences.end()) mValidSequences.insert(s);
      }
    }
  }

  bool isValid(int s) const { return (mValidSequences.find(s) != mValidSequences.end()); }
...
class TFusedMHAKernelFactory {
 public:
  const TFusedMHAKernelList* getXMMAKernels(
      const typename TFusedMHAKernelList::KernelMeta* pKernelList, unsigned int nbKernels,
      Data_type type, unsigned int sm) {
...
using FusedMultiHeadAttentionXMMAKernel =
    TFusedMultiHeadAttentionXMMAKernel<FusedMultiHeadAttentionKernelMetaInfoV1,
