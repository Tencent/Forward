...
__global__ void maskedSoftmax(const float rsqrtHeadSize, const T* input, T* output,
                              const int* maskIdx) {
...
__global__ void scaledSoftmaxKernelSmall(const int ld, const float rsqrtHeadSize, const T* input,
                                         T* output) {
  scaledSoftmaxSmall<T, TPB>(ld, ld, rsqrtHeadSize, input, output);
}

template <typename T, unsigned TPB>
__global__ void scaledSoftmaxKernel(const int ld, const float rsqrtHeadSize, const T* input,
                                    T* output) {
  scaledSoftmax<T, TPB>(ld, ld, rsqrtHeadSize, input, output);
}

template <typename T>
int computeScaledSoftmax(cudaStream_t stream, const int ld, const int B, const int N,
...
template <typename T, unsigned TPB>
__global__ void maskedScaledSoftmaxKernelSmall(const int ld, const float rsqrtHeadSize,
...
template <typename T, unsigned TPB>
__global__ void maskedScaledSoftmaxKernel(const int ld, const float rsqrtHeadSize,
                                          const int* maskIdx, const T* input, T* output) {
  __shared__ int lastValid;
...
template <typename T>
int computeMaskedScaledSoftmax(cudaStream_t stream, const int ld, const int B, const int N,
                               const float rsqrtHeadSize, const int* maskIdx, const T* input,
                               T* output) {
...
std::pair<int, int> tuneBatchedGemm(const int B, const int S, const int numHeads,
                                    const int headSize) {
  const int nruns = 500;
  ...
  for (int a = startAlgo; a <= endAlgo; a++) {
    ...
    for (int r = 0; r < nruns; r++) {
      CUBLASASSERT(cublasGemmStridedBatchedEx<T>(
          cublas, CUBLAS_OP_T, CUBLAS_OP_N, S, S, headSize, T(1.f), kptr, ldQKV, strideQKV, qptr,
          ldQKV, strideQKV, T(0.f), qkptr, S, omatSize, numMats, algo));
    }
    ...
    cudaEventRecord(start, stream);
    for (int r = 0; r < nruns; r++) {
      CUBLASASSERT(cublasGemmStridedBatchedEx<T>(cublas, CUBLAS_OP_N, CUBLAS_OP_N, headSize, S, S,
                                                 1.f, vptr, ldQKV, strideQKV, qkptr, S, omatSize,
                                                 0.f, output, ldOut, strideOut, numMats, algo));
    }
...
template int computeScaledSoftmax<float>(cudaStream_t stream, const int ld, const int B,
                                         const int N, const float rsqrtHeadSize, const float* input,
                                         float* output);
...
template int computeMaskedScaledSoftmax<float>(cudaStream_t stream, const int ld, const int B,
                                               const int N, const float rsqrtHeadSize,
                                               const int* maskIdx, const float* input,
                                               float* output);
template int computeMaskedScaledSoftmax<half>(cudaStream_t stream, const int ld, const int B,
                                              const int N, const float rsqrtHeadSize,
                                              const int* maskIdx, const half* input, half* output);

size_t MHARunner::getSerializationSize() const { return sizeof(mS) + sizeof(mB); }

void MHARunner::serialize(void* buffer) const {
  fwd::serialize_value(&buffer, mS);
  fwd::serialize_value(&buffer, mB);
}

void MHARunner::deserialize(const void* data, size_t length) {
  fwd::deserialize_value(&data, &length, &mS);
  fwd::deserialize_value(&data, &length, &mB);
  setup(mS, mB);
}

UnfusedMHARunner::UnfusedMHARunner(const nvinfer1::DataType type, const int numHeads,
                                   const int headSize)
...
UnfusedMHARunner::~UnfusedMHARunner() { CUBLASASSERT(cublasDestroy(mCublas)); }
...
void UnfusedMHARunner::serialize(void* buffer) const {
  fwd::serialize_value(&buffer, mAlgoBatchedEx1);
  fwd::serialize_value(&buffer, mAlgoBatchedEx2);
  MHARunner::serialize(buffer);
}

void UnfusedMHARunner::deserialize(const void* data, size_t length) {
  mIsBestAlgoFound = true;
  fwd::deserialize_value(&data, &length, &mAlgoBatchedEx1);
  fwd::deserialize_value(&data, &length, &mAlgoBatchedEx2);
  MHARunner::deserialize(data, length);
}

void UnfusedMHARunner::setup(const int S, const int B) {
  MHARunner::setup(S, B);
  if (mType == nvinfer1::DataType::kHALF && !mIsBestAlgoFound) {
    std::tie(mAlgoBatchedEx1, mAlgoBatchedEx2) = tuneBatchedGemm(B, S, mNumHeads, mHeadSize);
    mIsBestAlgoFound = true;

    LOG(INFO) << "QKV Plugin - Selected Algos for batch gemms: " << mAlgoBatchedEx1 << ", "
              << mAlgoBatchedEx2 << "\n";
  }
}

size_t UnfusedMHARunner::getWorkspaceSize() const { return 2UL * mWordSize * mOmatSize * mNumMats; }
...
void UnfusedMHARunner::run(const PluginTensorDesc& inputDesc, const PluginTensorDesc& outputDesc,
                           const void* qkvPtr, const void* maskPtr, void* output, void* workspace,
                           cudaStream_t stream) {
  ...
  if (mType == nvinfer1::DataType::kHALF) {
    ...
    CUBLASASSERT(::cublasGemmStridedBatchedEx(
        mCublas, CUBLAS_OP_T, CUBLAS_OP_N, mS, mS, mHeadSize, &alpha, kptr, CUDA_R_16F, mLdQKV,
        mStrideQKV, qptr, CUDA_R_16F, mLdQKV, mStrideQKV, &beta, qkptr, CUDA_R_16F, mS, mOmatSize,
        mNumMats, CUDA_R_16F, static_cast<cublasGemmAlgo_t>(mAlgoBatchedEx1)));

    // apply softmax
    if (maskIdx) {  // if we have a mask
      computeMaskedScaledSoftmax<half>(stream, mS, mB, mNumHeads, mRsqrtHeadSize, maskIdx, qkptr,
                                       pptr);
    ...
    // compute P*V (as V*P)
    CUBLASASSERT(cublasGemmStridedBatchedEx(
        mCublas, CUBLAS_OP_N, CUBLAS_OP_N, mHeadSize, mS, mS, &alpha, vptr, CUDA_R_16F, mLdQKV,
        mStrideQKV, pptr, CUDA_R_16F, mS, mOmatSize, &beta, output, CUDA_R_16F, mLdOut, mStrideOut,
        mNumMats, CUDA_R_16F, static_cast<cublasGemmAlgo_t>(mAlgoBatchedEx2)));
  } else {
    ...
    CUBLASASSERT(cublasGemmStridedBatched<float>(
        mCublas, CUBLAS_OP_T, CUBLAS_OP_N, mS, mS, mHeadSize, 1.f, kptr, mLdQKV, mStrideQKV, qptr,
        mLdQKV, mStrideQKV, 0.f, qkptr, mS, mOmatSize, mNumMats));

    // apply softmax
    if (maskIdx) {  // if we have a mask
      computeMaskedScaledSoftmax<float>(stream, mS, mB, mNumHeads, mRsqrtHeadSize, maskIdx, qkptr,
                                        pptr);
    } else {  // if we don't have a mask
    ...
    CUBLASASSERT(cublasGemmStridedBatched<float>(
        mCublas, CUBLAS_OP_N, CUBLAS_OP_N, mHeadSize, mS, mS, 1.f, vptr, mLdQKV, mStrideQKV, pptr,
        mS, mOmatSize, 0.f, outptr, mLdOut, mStrideOut, mNumMats));
  }
}

bool UnfusedMHARunner::isValid(int s) const { return mType != nvinfer1::DataType::kINT8; }
...
class FusedMHARunnerFP16::mhaImpl {
 public:
  mhaImpl(FusedMHARunnerFP16* interface)
      : interface(interface), sm(interface->mSm), xmmaKernel(getXMMAKernels(DATA_TYPE_FP16, sm)) {
  ...
  void setup(const int S, const int B) {
    // TODO these implementation details might be better centralized into the
    // XMMA code, since they are needed in several places (also outside of this
    // plugin)
    ...
    // The number of xmmas in the M dimension. We use one uint32_t per XMMA in
    // the M dimension.
  ...
  void run(const PluginTensorDesc& inputDesc, const PluginTensorDesc& outputDesc,
           const void* qkvPtr, const void* maskPtr, void* output, void* workspace,
           cudaStream_t stream) {
  ...
  bool isValid(int s) const { return xmmaKernel->isValid(s); }
...
FusedMHARunnerFP16::FusedMHARunnerFP16(const int numHeads, const int headSize, const int sm)
    : MHARunner(nvinfer1::DataType::kHALF, numHeads, headSize), mSm(sm), pimpl(new mhaImpl(this)) {}
...
size_t FusedMHARunnerFP16::getWorkspaceSize() const { return 0; }
...
void FusedMHARunnerFP16::run(const PluginTensorDesc& inputDesc, const PluginTensorDesc& outputDesc,
                             const void* qkvPtr, const void* maskPtr, void* output, void* workspace,
                             cudaStream_t stream) {
  pimpl->run(inputDesc, outputDesc, qkvPtr, maskPtr, output, workspace, stream);
}

void FusedMHARunnerFP16::run(const nvinfer1::PluginTensorDesc* inputDesc,
...
bool FusedMHARunnerFP16::isValid(int s) const { return pimpl->isValid(s); }
...
FusedMHARunnerInt8::FusedMHARunnerInt8(const int numHeads, const int headSize, const int sm,
                                       const float dqProbs)
    : MHARunner(nvinfer1::DataType::kINT8, numHeads, headSize),
      mSm(sm),
...
size_t FusedMHARunnerInt8::getWorkspaceSize() const { return 0; }
...
void FusedMHARunnerInt8::run(const PluginTensorDesc& inputDesc, const PluginTensorDesc& outputDesc,
                             const void* qkvPtr, const void* maskPtr, void* output, void* workspace,
                             cudaStream_t stream) {
  pimpl->run(inputDesc, outputDesc, qkvPtr, maskPtr, output, workspace, stream);
}

void FusedMHARunnerInt8::run(const nvinfer1::PluginTensorDesc* inputDesc,
                             const nvinfer1::PluginTensorDesc* outputDesc,
                             const void* const* inputs, void* const* outputs, void* workspace,
                             cudaStream_t stream) {
  assert(false && "not implemented");
...
bool FusedMHARunnerInt8::isValid(int s) const { return pimpl->isValid(s); }
...
class FusedMHARunnerFP16v2::mhaImpl {
 public:
  mhaImpl(FusedMHARunnerFP16v2* interface)
    ...
    assert((sm == kSM_72 || sm == kSM_75 || sm == kSM_80 || sm == kSM_86) &&
  ...
  void run(const PluginTensorDesc& inputDesc, const PluginTensorDesc& outputDesc,
           const void* qkvPtr, const void* maskPtr, const void* cuSeqlenPtr, void* output,
...
size_t FusedMHARunnerFP16v2::getWorkspaceSize() const { return 0; }
...
void FusedMHARunnerFP16v2::run(const PluginTensorDesc& inputDesc,
                               const PluginTensorDesc& outputDesc, const void* qkvPtr,
                               const void* maskPtr, void* output, void* workspace,
                               cudaStream_t stream) {
  assert(false && "not implemented");
  // pimpl->run(inputDesc, outputDesc, qkvPtr, maskPtr, output, workspace,
  // stream);
}
...
void FusedMHARunnerFP16v2::run(const nvinfer1::PluginTensorDesc* inputDesc,
                               ...
                               const void* const* inputs, void* const* outputs, void* workspace,
                               cudaStream_t stream) {
  pimpl->run(inputDesc[0], outputDesc[0], inputs[0], inputs[1], inputs[2], outputs[0], workspace,
             stream);
}

bool FusedMHARunnerFP16v2::isValid(int s) const { return pimpl->isValid(s); }
...
size_t FusedMHARunnerInt8v2::getWorkspaceSize() const { return 0; }
...
void FusedMHARunnerInt8v2::run(const PluginTensorDesc& inputDesc,
                               const PluginTensorDesc& outputDesc, const void* qkvPtr,
                               const void* maskPtr, void* output, void* workspace,
                               cudaStream_t stream) {
  assert(false && "Not implemented");
